package chapter7.exercises.knightTourVarieties.knightsTourMultiPlayer;
...
public class ChessBoard {
   ...
   static boolean  winnerFound=false;
    ...
    private static chapter7.exercises.knightTourVarieties.knightsTourMultiPlayer.Player[] players = new chapter7.exercises.knightTourVarieties.knightsTourMultiPlayer.Player[2];
static boolean isValidCapture=false;
    public static void displayIntroMessage() {
        ...
        public static void setPlayer(int currentPlayer, String name) {
            if (currentPlayer==0){
               players[currentPlayer]=new chapter7.exercises.knightTourVarieties.knightsTourMultiPlayer.Player(name,"K1");
            }    else {
        ...
        public static void registerPlayers(Scanner scanner){
            for (int currentPlayer=0;currentPlayer<2;currentPlayer++){
                ...
                System.out.println("Player "+(currentPlayer+1)+" has been successfully registered");
            ...
            for (int eachRound=0;eachRound<players.length;eachRound++) {
               ...
               Player opponent=players[1-eachRound];
                int[][] validMoves = getPossibleMoves(row, column,opponent.getCurrentRowPosition(),opponent.getCurrentColumnPosition());
                 showBoardWithValidMoves(validMoves,currentPlayer,roundIndex,validCaptureNumber);
                ...
                if (gameIsWon(validCaptureNumber,playerMoveIndex)) {
...
public static void showBoardWithValidMoves(int[][] validMoves,Player player,int roundIndex,int validCaptureNumber) {
...
public static boolean gameIsWon(int validCaptureNumber,int moveNumber){
   if (isValidCapture&&validCaptureNumber==moveNumber){
      return true;   }
   return false;
}
